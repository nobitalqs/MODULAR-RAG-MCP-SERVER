...
- **熔断与 Provider 容错 (Circuit Breaker & Failover)**：
    - 三态熔断器（CLOSED → OPEN → HALF_OPEN）保护外部服务调用；
    - ProviderChain 多 Provider 级联容错，主备自动切换。
...
#### 3.3.8 熔断与 Provider 容错 (Circuit Breaker & Provider Failover)

当 LLM Provider 出现连续故障时，熔断器自动切断请求（避免雪崩），ProviderChain 自动切换到备用 Provider，保障系统可用性。
...
> **当前实现说明**：已实现 `CircuitBreaker`（线程安全，`threading.Lock`）和 `ProviderChain`。通过 `LLMFactory.create_with_failover(settings)` 创建——如果配置了 `fallback_providers` 则返回 ProviderChain，否则返回带熔断器包装的单 Provider。
...
│   │   │   ├── pdf_loader.py            # PDF Loader (MarkItDown + 表格/公式提取)
...
│   │   │   └── redis_limiter.py         # Redis 滑动窗口限流器
│   │   │
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ LLM 答案生成    │  via ProviderChain (熔断器保护 + 自动容错)
│ [Circuit Breaker]│
└────────┬────────┘
         │
         ▼
...
retrieval:
  ...
  sparse_weight: 1.0        # Sparse (BM25) 检索权重 (>= 0.0)
