...
class ResponseBuilder:
    ...
    def __init__(
        ...
        multimodal_assembler: Optional[Any] = None,
    ...
    @property
    def multimodal_assembler(self) -> Optional[Any]:
        """Get or create MultimodalAssembler instance.

        Returns None if the multimodal_assembler module is not yet available (E6).
        """
        if self._multimodal_assembler is None:
            try:
                ...
                logger.debug("MultimodalAssembler not available (E6 not implemented)")
                return None
    ...
    def build(
        ...
        # Assemble image content if enabled and assembler available
        image_contents: List[types.ImageContent] = []
        assembler = self.multimodal_assembler
        if self.enable_multimodal and include_images and assembler is not None:
            image_blocks = assembler.assemble(results, collection)
