...
class SQLiteIntegrityChecker(FileIntegrityChecker):
    ...
    def lookup_by_path(self, file_path: str, collection: str | None = None) -> str | None:
        ...
        """
        query = "SELECT file_hash FROM ingestion_history WHERE file_path = ? AND status = 'success'"
    ...
    def mark_success(self, file_hash: str, file_path: str, collection: str | None = None) -> None:
        ...
        try:
            ...
            cursor = conn.execute(
                "SELECT processed_at FROM ingestion_history WHERE file_hash = ?", (file_hash,)
            )
            result = cursor.fetchone()
