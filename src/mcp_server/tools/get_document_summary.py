...
class GetDocumentSummaryTool:
    ...
    def _find_document_chunks(
        ...
        Uses two strategies:
        1. Search by source_ref metadata (preferred, exact match)
        2. Fallback: partial matching on chunk IDs
        ...
        try:
            ...
            logger.debug("source_ref search failed: %s", e)
        ...
        try:
            ...
            logger.debug("ID prefix search failed: %s", e)
    ...
    async def execute(
        ...
        """
        logger.info(
            "Executing get_document_summary (doc_id=%s, collection=%s)",
            doc_id, collection,
        )
