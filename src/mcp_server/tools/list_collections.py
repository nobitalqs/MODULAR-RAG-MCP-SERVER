...
class ListCollectionsTool:
    ...
    def _get_chroma_client(self) -> Any:
        ...
        if not persist_path.exists():
            logger.warning("ChromaDB directory does not exist: %s", persist_path)
    ...
    def list_collections(
        ...
        try:
            ...
            logger.error("Failed to get ChromaDB client: %s", e)
            return []
            ...
            for collection in collections:
                ...
                if include_stats:
                    try:
                        ...
                        logger.warning(
                            "Failed to get count for collection '%s': %s",
                            collection.name, e,
                        )
                        info.count = None
        ...
        logger.info("Found %d collections", len(collections_info))
    ...
    def format_response(
        ...
        Returns:
            Formatted Markdown string suitable for MCP response.
        """
        if not collections:
        ...
        for i, coll in enumerate(collections, 1):
            ...
            if coll.metadata:
                ...
                if user_metadata:
                    meta_str = ", ".join(
                        f"{k}={v}" for k, v in user_metadata.items()
                    )
                    line += f" ({meta_str})"
    ...
    async def execute(
        ...
        """
        logger.info("Executing list_collections (include_stats=%s)", include_stats)
        ...
        try:
            ...
            return types.CallToolResult(
                content=[
                    types.TextContent(
                        type="text",
                        text=f"Error listing collections: {e}",
                    )
                ],
                isError=True,
            )
