...
class TestBM25IndexerBasics:
    ...
    def test_indexer_initialization_default(self):
        ...
        assert os.path.normpath(str(indexer.index_dir)) == os.path.normpath(
            "data/db/bm25"
        )

    def test_indexer_initialization_custom(self):
        """Test initialization with custom parameters."""
        indexer = BM25Indexer(index_dir="custom/path", k1=2.0, b=0.5)
...
class TestBM25IndexBuilding:
    ...
    def test_build_invalid_structure_raises(self, tmp_path):
        ...
        with pytest.raises(
            ValueError, match="missing required field: term_frequencies"
        ):
...
class TestIDFCalculation:
    ...
    def test_idf_calculation_formula(self, tmp_path):
        """Test IDF = log((N - df + 0.5) / (df + 0.5))."""
        indexer = BM25Indexer(index_dir=str(tmp_path))

        # N=3, df=1 → IDF = log((3-1+0.5)/(1+0.5)) = log(2.5/1.5)
    ...
    def test_idf_rare_vs_common_terms(self, tmp_path):
        ...
        term_stats = [
            {
                "chunk_id": "1",
                "term_frequencies": {"rare": 1, "common": 1},
                "doc_length": 2,
            },
...
class TestBM25Querying:
    ...
    def test_query_returns_sorted_results(self, tmp_path):
        ...
        term_stats = [
            {
                "chunk_id": "relevant",
                "term_frequencies": {"machine": 3, "learning": 2},
                "doc_length": 5,
            },
            {
                "chunk_id": "partial",
                "term_frequencies": {"machine": 1},
                "doc_length": 3,
            },
            {
                "chunk_id": "irrelevant",
                "term_frequencies": {"python": 2},
                "doc_length": 2,
            },
        ]
...
class TestRemoveDocument:
    ...
    def test_remove_existing_document(self, tmp_path):
        """Test removing a document that exists in the index."""
        indexer = BM25Indexer(index_dir=str(tmp_path))
        ...
        indexer.build(term_stats, collection="test")
        removed = indexer.remove_document("doc1", collection="test")

        assert removed is True

        # doc1 chunks should be gone
        results = indexer.query(["world"], top_k=10)
        assert len(results) == 0  # "world" only in doc1

        # doc2 should remain
        results = indexer.query(["python"], top_k=10)
        assert len(results) == 1
        assert results[0]["chunk_id"] == "doc2_chunk0"

    def test_remove_nonexistent_document(self, tmp_path):
        """Test removing a document that doesn't exist returns False."""
        indexer = BM25Indexer(index_dir=str(tmp_path))
        ...
        removed = indexer.remove_document("nonexistent", collection="test")
...
class TestEdgeCases:
    ...
    def test_single_document_corpus(self, tmp_path):
        ...
        Note: IDF can be negative for single-doc corpus:
        log((1-1+0.5)/(1+0.5)) = log(0.33) < 0. This is expected BM25 behavior.
        """
        indexer = BM25Indexer(index_dir=str(tmp_path))
    ...
    def test_empty_document(self, tmp_path):
        """Test handling document with zero terms."""
        indexer = BM25Indexer(index_dir=str(tmp_path))
