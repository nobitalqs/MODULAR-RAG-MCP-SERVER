"""Tests for CoreReranker fallback behavior.
    ...
    def rerank(
        ...
        **kwargs: Any,
        ...
        for i, candidate in enumerate(reversed(candidates)):
            ...
            candidate_copy["rerank_score"] = 3.0 - i * 0.5
...
class FailingReranker(BaseReranker):
    ...
    def __init__(
        self,
        settings: Any = None,
        error_message: str = "Reranker failed",
        **kwargs: Any,
    ):
...
class TestCoreRerankerNormalFlow:
    ...
    def test_rerank_explicit_top_k_overrides_config(
        self, mock_settings, sample_results,
    ):
...
class TestCoreRerankerFallback:
    ...
    def test_fallback_on_backend_error(self, mock_settings, sample_results):
        ...
        reranker = CoreReranker(
            settings=mock_settings, reranker=failing_reranker,
        )
    ...
    def test_fallback_preserves_original_order(
        self, mock_settings, sample_results,
    ):
    ...
    def test_stores_original_order_on_fallback(
        self, mock_settings, sample_results,
    ):
