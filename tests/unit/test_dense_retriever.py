...
class FakeEmbedding:
    ...
    def embed(
        self, texts: List[str], trace: Any = None, **kwargs: Any,
    ) -> List[List[float]]:
        """Return deterministic fake embeddings."""
        self.call_count += 1
        self.last_texts = texts
        return [
            [float(i) / self.dimension for i in range(self.dimension)]
            for _ in texts
        ]
...
class FakeVectorStore:
    ...
    def __init__(self, stored_records: List[Dict[str, Any]] | None = None):
...
class TestDenseRetrieverRetrieve:
    ...
    def test_retrieve_passes_trace(self, retriever, fake_embedding):
...
class TestDenseRetrieverErrorHandling:
    ...
    def test_retrieve_handles_malformed_results(self, fake_embedding):
        ...
        mock_store.query.return_value = [
            ...
            {"id": "", "score": "invalid", "text": ""},  # Malformed: empty chunk_id
...
class TestCreateDenseRetriever:
    ...
    def test_create_with_injected_dependencies(
        self, fake_embedding, fake_vector_store,
    ):
    ...
    def test_create_without_settings(self, fake_embedding, fake_vector_store):
        """Test factory without settings uses default top_k."""
        retriever = create_dense_retriever(
            embedding_client=fake_embedding,
            vector_store=fake_vector_store,
        )
