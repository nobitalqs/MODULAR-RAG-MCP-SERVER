...
class TestRRFFusionBasic:
    ...
    def test_fuse_two_overlapping_lists(
        self, fusion_default, dense_results, sparse_results,
    ):
    ...
    def test_fuse_disjoint_lists(
        self, fusion_default, disjoint_results_1, disjoint_results_2,
    ):
        ...
        assert abs(x_result.score - 1 / 61) < 1e-10

    def test_fuse_with_top_k(
        self, fusion_default, dense_results, sparse_results,
    ):
...
class TestRRFFusionDeterministic:
    ...
    def test_list_order_affects_metadata_source(self, fusion_default):
        ...
        list1 = [
            RetrievalResult(
                ...
                text="Text from list 1", metadata={"source": "list1"},
            ),
        ]
        list2 = [
            RetrievalResult(
                chunk_id="shared", score=5.0,
                text="Text from list 2", metadata={"source": "list2"},
            ),
        ]
    ...
    def test_order_of_lists_does_not_affect_scores(
        self, fusion_default, dense_results, sparse_results,
    ):
        """Swapping list order should produce same scores."""
        fused1 = fusion_default.fuse([dense_results, sparse_results])
...
class TestRRFFusionKParameter:
    ...
    def test_k_affects_final_ranking(self):
        """Different k values can lead to different score magnitudes."""
        list1 = [
...
class TestRRFFusionWeighted:
    ...
    def test_weighted_fusion_weight_length_mismatch(
        self, fusion_default, dense_results, sparse_results,
    ):
    ...
    def test_weighted_fusion_negative_weight_rejected(
        self, fusion_default, dense_results,
    ):
    ...
    def test_weighted_fusion_zero_weight(self, fusion_default):
        ...
        assert abs(scores["b"] - 0.0) < 1e-10

    def test_weighted_fusion_with_top_k(
        self, fusion_default, dense_results, sparse_results,
    ):
...
class TestRRFFusionRealisticScenarios:
    ...
    def test_typical_hybrid_search_scenario(self, fusion_default):
        ...
        # both_1: 1/(60+3) + 1/(60+2) ≈ 0.0320
        # semantic_1: 1/(60+1) ≈ 0.0164
