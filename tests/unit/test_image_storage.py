...
class TestImageStorageInitialization:
    """Test initialization and schema setup."""
    ...
    def test_database_indexes_created(self, temp_storage):
        ...
        cursor = conn.execute(
            "SELECT name FROM sqlite_master "
            "WHERE type='index' AND name='idx_doc_hash'"
        )
        assert cursor.fetchone() is not None
        conn.close()
...
class TestSaveImage:
    ...
    def test_save_image_creates_collection_directory(
        self, temp_storage, sample_image_data
    ):
        path = temp_storage.save_image(
            image_id="img1",
            image_data=sample_image_data,
    ...
    def test_save_image_with_custom_extension(
        self, temp_storage, sample_image_data
    ):
        path = temp_storage.save_image(
            image_id="img1",
    ...
    def test_save_image_registers_in_database(
        self, temp_storage, sample_image_data
    ):
        ...
        conn = sqlite3.connect(temp_storage.db_path)
        cursor = conn.execute(
            "SELECT image_id, collection, doc_hash, page_num "
    ...
    def test_save_image_idempotent(self, temp_storage, sample_image_data):
        ...
        conn = sqlite3.connect(temp_storage.db_path)
        count = conn.execute(
            "SELECT COUNT(*) FROM image_index WHERE image_id = ?", ("img1",)
        ).fetchone()[0]
        conn.close()
    ...
    def test_save_image_empty_id_raises_error(
        self, temp_storage, sample_image_data
    ):
    ...
    def test_save_image_missing_source_file_raises_error(self, temp_storage):
        with pytest.raises(IOError, match="Failed to save image"):
            temp_storage.save_image(
                image_id="img1", image_data="/nonexistent/path.png"
            )

    def test_save_image_uses_default_collection(
        self, temp_storage, sample_image_data
    ):
        path = temp_storage.save_image(
            image_id="img1", image_data=sample_image_data
        )
...
class TestGetImagePath:
    ...
    def test_get_existing_image_path(self, temp_storage, sample_image_data):
        original = temp_storage.save_image(
            image_id="img1", image_data=sample_image_data
        )
        assert temp_storage.get_image_path("img1") == original

    def test_get_nonexistent_image_path_returns_none(self, temp_storage):
        assert temp_storage.get_image_path("nonexistent") is None
...
class TestImageExists:
    ...
    def test_image_exists_returns_true(self, temp_storage, sample_image_data):
        temp_storage.save_image(image_id="img1", image_data=sample_image_data)
...
class TestListImages:
    ...
    def test_list_all_images(self, temp_storage, sample_image_data):
        ...
        assert len(images) == 3
        assert {img["image_id"] for img in images} == {"img1", "img2", "img3"}
    ...
    def test_list_images_by_collection(self, temp_storage, sample_image_data):
        ...
        assert len(images) == 2
        assert {img["image_id"] for img in images} == {"img1", "img3"}

    def test_list_images_by_doc_hash(self, temp_storage, sample_image_data):
        temp_storage.save_image("img1", sample_image_data, "c", doc_hash="h1")
        temp_storage.save_image("img2", sample_image_data, "c", doc_hash="h2")
        temp_storage.save_image("img3", sample_image_data, "c", doc_hash="h1")

        images = temp_storage.list_images(doc_hash="h1")

        assert len(images) == 2
        assert {img["image_id"] for img in images} == {"img1", "img3"}

    def test_list_images_with_both_filters(
        self, temp_storage, sample_image_data
    ):
        temp_storage.save_image("img1", sample_image_data, "c1", doc_hash="h1")
        temp_storage.save_image("img2", sample_image_data, "c2", doc_hash="h1")
        temp_storage.save_image("img3", sample_image_data, "c1", doc_hash="h2")

        images = temp_storage.list_images(collection="c1", doc_hash="h1")

        assert len(images) == 1
        assert images[0]["image_id"] == "img1"

    def test_list_images_returns_complete_metadata(
        self, temp_storage, sample_image_data
    ):
        temp_storage.save_image(
            "img1", sample_image_data, "test", doc_hash="abc123", page_num=5
        )
    ...
    def test_list_images_empty_when_no_matches(self, temp_storage):
        assert temp_storage.list_images(collection="nonexistent") == []
...
class TestDeleteImage:
    ...
    def test_delete_image_removes_from_database(
        ...
        assert temp_storage.delete_image("img1") is True
    ...
    def test_delete_image_keeps_file_when_requested(
        self, temp_storage, sample_image_data
    ):
        path = temp_storage.save_image("img1", sample_image_data)
    ...
    def test_delete_nonexistent_image_returns_false(self, temp_storage):
        assert temp_storage.delete_image("nonexistent") is False


class TestGetCollectionStats:
    """Test collection statistics."""

    def test_counts_images(self, temp_storage, sample_image_data):
        temp_storage.save_image("img1", sample_image_data, "coll1")
    ...
    def test_calculates_size(self, temp_storage, sample_image_data):
        temp_storage.save_image("img1", sample_image_data, "coll1")
        temp_storage.save_image("img2", sample_image_data, "coll1")
    ...
    def test_empty_collection(self, temp_storage):
...
class TestConcurrency:
    ...
    def test_multiple_reads(self, temp_storage, sample_image_data):
        temp_storage.save_image("img1", sample_image_data)
        assert temp_storage.get_image_path("img1") == temp_storage.get_image_path("img1")

    def test_write_during_read(self, temp_storage, sample_image_data):
        temp_storage.save_image("img1", sample_image_data)
...
class TestEdgeCases:
    ...
    def test_special_characters_in_image_id(
        self, temp_storage, sample_image_data
    ):
        ...
        assert temp_storage.get_image_path(image_id) == path
    ...
    def test_close_and_reopen(self, sample_image_data):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(storage2.list_images()) == 1
            storage2.close()
