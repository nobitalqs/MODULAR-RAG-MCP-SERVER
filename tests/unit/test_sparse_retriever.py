...
class MockBM25Indexer:
    ...
    def load(
        self, collection: str = "default", trace: Optional[Any] = None,
    ) -> bool:
...
class MockBM25IndexerFailing:
    ...
    def __init__(
        self, fail_on_load: bool = False, fail_on_query: bool = False,
    ) -> None:
...
class MockSettings:
    ...
    def __init__(self, sparse_top_k: int = 15) -> None:
...
class TestCreateSparseRetriever:
    ...
    def test_create_without_settings(self):
        """Test factory without settings uses defaults."""
        bm25 = MockBM25Indexer()
        vs = MockVectorStore()
...
class TestSparseRetrieverIndexLoading:
    ...
    def test_index_loaded_once_per_collection(self):
        """Test that index is loaded for each retrieve call."""
        bm25 = MockBM25Indexer()
        vs = MockVectorStore()
        retriever = SparseRetriever(bm25_indexer=bm25, vector_store=vs)
